...
generate-plugins:
	...
	@if [ -s "$(ADAPTERS)" ] && grep -q '^[^#]' "$(ADAPTERS)" 2>/dev/null; then \
		...
		grep -v '^#' "$(ADAPTERS)" | grep -v '^$$' | while read -r mod ver; do \
			if [ "$$ver" = "latest" ]; then \
				printf '[>] Resolving latest commit for %s...\n' "$$mod"; \
				commit=$$(git ls-remote "https://$$mod.git" HEAD | cut -f1 | cut -c1-12); \
				if [ -z "$$commit" ]; then \
					printf '[ERROR] Failed to resolve latest commit for %s\n' "$$mod"; \
					exit 1; \
				fi; \
				ver="$$commit"; \
				printf '[>] Resolved to commit: %s\n' "$$commit"; \
			fi; \
