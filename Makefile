...
help:
	...
	@printf '  make run\n'
	@printf '  make build\n'
	@printf '  make add-adapter MODULE=github.com/user/adapter QUERY=main\n'
...
generate-plugins:
	...
	@mkdir -p plugins
	@if [ ! -f "$(ADAPTERS)" ]; then \
		printf '# External adapter registry\n# Format: <module-path> <module-query>\n# Example: github.com/user/adapter main\n# Example: github.com/user/adapter v1.2.0\n# Example: github.com/user/adapter abc123def456\n' > "$(ADAPTERS)"; \
	fi
...
add-adapter:
	@if [ -z "$(MODULE)" ]; then \
		printf 'Usage: make add-adapter MODULE=github.com/user/adapter QUERY=main\n'; \
		exit 1; \
	fi
	@query='$(or $(QUERY),$(VERSION))'; \
	if [ -z "$$query" ]; then query='main'; fi; \
	printf '$(MODULE) %s\n' "$$query" >> "$(ADAPTERS)"; \
	printf '[OK] Added $(MODULE)@%s to $(ADAPTERS)\n' "$$query"; \
	if [ -n "$(VERSION)" ] && [ -z "$(QUERY)" ]; then printf '[!] VERSION is deprecated; use QUERY next time.\n'; fi
