...
func main() {
	...
	fmt.Printf("  smoke             Restart with NO_AUTH=1, then run the smoke harness (wire surfaces)\n")
	...
	fmt.Printf("  go-test           Run go test (PKG, default \"%s\")\n", pkg)
	fmt.Printf("  fmt               Apply gofmt to both Go modules\n")
	fmt.Printf("  fmt-check         Fail when gofmt would reformat anything\n")
	...
	fmt.Printf("  SMOKE_PLUGINS      Comma plugin types for smoke (or \"all\"). Default: \"mock\"\n")
	fmt.Printf("  SMOKE_TARGETS      Comma targets for smoke. Default: \"completions,messages\"\n")
